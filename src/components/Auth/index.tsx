...
const ERR_MAP: Record<string, string> = {
  ...
  'auth/popup-closed-by-user': 'חלון הכניסה נסגר — נסה שוב',
...
export const AuthScreen: React.FC = () => {
  ...
  // בדוק redirect result מ-Google (למובייל)
  useEffect(() => {
    checkGoogleRedirect().catch(() => {});
  }, []);
  ...
  const handleSubmit = async () => {
    ...
    if (mode === 'register' && !consent) { setError('נא לאשר את תנאי השימוש'); return; }
    setLoading(true);
    try {
      if (mode === 'login') {
        const cred = await authEmailSignIn(email, password);
        if (!cred.user.emailVerified) { setVerifyMode(true); setLoading(false); return; }
      } else {
  ...
  const handleGoogle = async () => {
    ...
    try {
      ...
      setError(ERR_MAP[e.code] || 'שגיאה בהתחברות עם Google — נסה שוב');
    }
    setLoading(false);
  };
  ...
  const handleForgot = async () => {
    ...
    setLoading(true);
    try { await authForgot(email); setForgotSent(true); }
    catch (e: any) { setError(ERR_MAP[e.code] || e.message); }
    setLoading(false);
  };
  ...
  const handleResend = async () => {
    ...
    setLoading(true);
    try { await authSendVerification(auth.currentUser); startCooldown(); }
    catch (e: any) { setError(ERR_MAP[e.code] || e.message); }
    setLoading(false);
  };
  ...
  const handleCheckVerified = async () => {
    ...
    if (!auth.currentUser.emailVerified) {
      setError('המייל עדיין לא אומת — בדוק את תיבת הדוא"ל שלך ולחץ על הקישור');
    }
    setLoading(false);
  };
    ...
    return (
      <div className="min-h-screen bg-surface flex items-center justify-center p-5">
        <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }}
          ...
          <h2 className="text-xl font-black text-on-surface mb-2">אמת את כתובת המייל</h2>
          <p className="text-sm text-on-surface-variant mb-1">שלחנו קישור אימות אל:</p>
          ...
          <div className="bg-primary/5 border border-primary/15 rounded-xl p-4 mb-6 text-right space-y-2">
            {['פתח את המייל שנשלח אליך', 'לחץ על קישור האימות', 'חזור כאן ולחץ "כבר אימתתי"'].map((s, i) => (
          ...
          <button onClick={handleResend} disabled={resendCooldown > 0 || loading}
            className="w-full py-2.5 bg-surface-container-high text-on-surface-variant rounded-xl text-sm font-medium disabled:opacity-50 flex items-center justify-center gap-2 mb-4">
            <RefreshCw size={14} />
            {resendCooldown > 0 ? `שלח שוב בעוד ${resendCooldown} שניות` : 'שלח מייל אימות שוב'}
          </button>
          <button onClick={() => { setVerifyMode(false); setError(''); setMode('login'); }}
      ...
      <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.25 }}
        ...
        <div className="flex gap-1.5 flex-wrap mb-6">
          {(Object.entries(LANGS) as [LangCode, typeof LANGS[LangCode]][]).map(([code, L]) => (
            <button key={code} onClick={() => setLangCode(code)}
              className={cn('px-2.5 py-1 rounded-full text-[11px] font-medium border transition-all flex items-center gap-1',
                langCode === code ? 'border-primary/40 bg-primary/10 text-primary' : 'border-outline-variant/20 text-on-surface-variant hover:border-outline-variant/40'
              )}>
              {L.flag} {L.name}
            </button>
          ))}
        ...
        {forgotSent ? (
          <div className="text-center py-4">
            ...
            <p className="text-xs text-on-surface-variant mt-1">בדוק את תיבת הדוא"ל</p>
            ...
            {mode === 'register' && (
              <div className="relative">
                ...
                <input className="w-full ps-10 pe-4 py-3 bg-surface-container-high border-0 rounded-xl text-sm text-on-surface focus:ring-2 focus:ring-primary/30"
                  placeholder="שם מלא" value={fullName} onChange={e => setFullName(e.target.value)} />
              </div>
            )}
            <div className="relative">
              ...
              <input type="email" dir="ltr"
                ...
                placeholder='כתובת דוא"ל' value={email} onChange={e => setEmail(e.target.value)} />
            </div>
            {mode !== 'forgot' && (
              <div className="relative">
                ...
                <input type={showPass ? 'text' : 'password'} dir="ltr"
                  ...
                  placeholder="סיסמה (לפחות 6 תווים)" value={password}
