...
import { Save, PlusCircle, Trash2, CreditCard, Wallet, AlertTriangle } from 'lucide-react';
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="bg-surface-container-low rounded-2xl p-7 shadow-sm border border-outline-variant/5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <select
              className="w-full bg-surface-container-lowest border-0 rounded-xl px-4 py-3 text-sm text-on-surface focus:ring-2 focus:ring-primary/20 transition-all"
              value={currency}
          ...
          <div>
            ...
            >
              {[1, 5, 10, 15, 20, 25].map(d => (
                <option key={d} value={d}>יום {d} לחודש</option>
              ))}
            </select>
          </div>
        </div>
        ...
        <div className="mt-6 flex items-center justify-between p-4 bg-surface-container-lowest rounded-xl border border-outline-variant/8">
          ...
          <label className="relative inline-flex items-center cursor-pointer">
            <input
              ...
              defaultChecked={s.budget.rollover !== false}
          ...
          {[
            ...
            <div key={i} className="bg-surface-container-lowest p-5 rounded-xl border border-outline-variant/8">
              ...
              <div className="flex items-center gap-2">
                ...
                <input
                  className="bg-transparent border-0 text-2xl font-black w-full focus:ring-0 p-0 text-on-surface"
                  type="number"
                  value={item.value || ''}
      ...
      <div className="bg-surface-container-low rounded-2xl p-7 shadow-sm border border-outline-variant/5">
        ...
        <div className="space-y-3">
          {(db.creditCards || []).map(card => (
            <div key={card.id} className="flex items-center justify-between p-4 bg-surface-container-high rounded-xl border border-outline-variant/8">
              <div>
                ...
                <div className="text-xs text-on-surface-variant">•••• •••• •••• {card.last4 || '****'} · יום חיוב {card.billingDay}</div>
              </div>
              <div className="flex items-center gap-3">
                <span className="text-xs text-primary font-bold">פעיל</span>
          ...
          {(db.creditCards || []).length === 0 && (
            <button
              onClick={addCard}
              className="w-full py-4 border-2 border-dashed border-outline-variant/30 rounded-xl text-on-surface-variant hover:border-primary/50 hover:text-primary transition-all text-sm font-medium flex items-center justify-center gap-2"
            >
      ...
      <motion.div
        ...
        animate={{ scale: saved ? [1, 1.05, 1] : 1 }}
      >
        <button
          onClick={handleSave}
          className={cn(
