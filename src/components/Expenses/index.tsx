...
import {
  fixedTotal, varTotal, varForMonth, getCurrencySymbol,
  today, uid,
...
const ExpenseRow: React.FC<{
  ...
  onEdit?: () => void;
  onDelete?: () => void;
}> = ({ name, category, amount, sym, sub, onEdit, onDelete }) => (
  <div className="flex items-center justify-between py-3.5 border-b border-outline-variant/6 last:border-0 group hover:bg-surface-container-highest/30 rounded-lg px-2 -mx-2 transition-colors">
    <div className="flex-1 min-w-0">
      ...
      <div className="text-[11px] text-on-surface-variant">{category}{sub ? ` · ${sub}` : ''}</div>
    </div>
    <div className="flex items-center gap-2 ms-3 flex-shrink-0">
      {/* Hidden actions on hover */}
      <div className="opacity-0 group-hover:opacity-100 flex gap-1 transition-opacity">
        {onEdit && (
          <button onClick={onEdit} className="p-1.5 rounded-lg hover:bg-primary/10 hover:text-primary text-on-surface-variant transition-colors">
            <Pencil size={13} />
          </button>
        )}
        {onDelete && (
          <button onClick={onDelete} className="p-1.5 rounded-lg hover:bg-error/10 hover:text-error text-on-surface-variant transition-colors">
            <Trash2 size={13} />
          </button>
        )}
      </div>
...
interface AddExpenseModalProps {
  type: ScopeType;
  isFixed: boolean;
  existing?: FixedExpense | VariableExpense;
  onClose: () => void;
}
  ...
  const handleSave = async () => {
    ...
    if (isFixed) {
      ...
      await updateDB(d => ({
        ...d,
        variable: existing
          ? d.variable.map(e => e.id === entry.id ? entry : e)
          : [...(d.variable || []), entry],
      }));
    }
    onClose();
      ...
      >
        ...
        <div className="space-y-4">
          <div>
            ...
            <input
              className="w-full bg-surface-container-low border-0 rounded-xl px-4 py-3 text-sm text-on-surface focus:ring-2 focus:ring-primary/30 transition-all"
        ...
        <div className="flex gap-3 mt-6">
          <button onClick={onClose} className="flex-1 py-3 bg-surface-container-high text-on-surface rounded-xl font-medium text-sm hover:bg-surface-container-highest transition-colors">
            {t(lang, 'cancel')}
          </button>
          <button
            onClick={handleSave}
            disabled={saving || !name.trim() || !amount}
            className="flex-1 py-3 bg-primary text-on-primary rounded-xl font-bold text-sm disabled:opacity-50 transition-all hover:shadow-lg hover:shadow-primary/20"
          >
            {saving ? '...' : t(lang, 'save')}
          </button>
        </div>
...
export const Expenses: React.FC<ExpensesProps> = ({ scope }) => {
  const { db, month, year, lang, updateDB } = useApp();
  const sym    = getCurrencySymbol(db);
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center justify-between p-6 border-b border-outline-variant/8">
          ...
          <div className="flex items-center gap-3">
            <span className="text-on-surface-variant text-sm font-medium">
              {sym}{Math.round(totalF).toLocaleString('he-IL')} {t(lang, 'total')}
            </span>
            <button
      ...
      <div className="bg-surface-container-low rounded-2xl border border-outline-variant/5">
        <div className="flex items-center justify-between p-6 border-b border-outline-variant/8">
          ...
          <div className="flex items-center gap-3">
            <span className="text-on-surface-variant text-sm font-medium">
              {sym}{Math.round(totalV).toLocaleString('he-IL')} {t(lang, 'total')}
            </span>
            <button
